...
# v2.1.3, Jul 2023 - Patch service manager refresh failure issue. Added trace#
#                    report to Convert execution on exception. Set 'ignore-  #
#                    DataItemCheck' property to False when 'GetTarget' action#
...
#                    Added 'thumbnail' to item backup/restore logic.         #
##############################################################################
...
def _getManager(item, verbose=None, outcome=None):
    ...
    if not item.type == "Feature Service":
        if verbose:
            print(
                "\n * Item is not a Feature Service or View: {}, '{}'".format(
                    item.id, item.title
                )
            )
        ...
        outcome["items"].append(
            {
                ...
                "itemType": item.type,
                "action": "get manager",
                "result": "Item Type is NOT a 'Feature Service' or 'Feature View'",
            }
        )
        ...
        manager = (
            arcgis.features.FeatureLayerCollection.fromitem(item).manager
...
def _asyncJob(
    service, endpoint, data, verbose=None, indent="", noWait=False, timeout=None
):
    ...
    sleepIntervals = {
        ...
        "23": 10,
        "29": 15,
    }  # Slowly increasing Job status query interval by changing the Sleep Time value at Sleep Cycle Intervals
    ...
    try:
        ...
        if isinstance(outcome, dict) and outcome.get("statusURL", None):
            ...
            while True:
                ...
                errorCode = (
                    outcome["error"].get("code", 0) if "error" in outcome else 399
                )
                errorDesc = (
                    outcome["error"].get("description", "N/A")
                    if "error" in outcome
                    else outcome
                )
                ...
                if noWait:
                    if verbose:
                        print(
                            " * No Wait specified * Manual Status URL: '{}'".format(
                                statusUrl
                            )
                        )
                ...
                sleepCycles += 1
                sleepTime = sleepIntervals.get(
                    str(sleepCycles), sleepTime
                )  # Adjust Sleep time by the interval cycles performed
                if verbose:
        ...
        outcome = {
            ...
            "error": {
                "code": 400,
                "message": "Error, '{}' request failed!".format(endpoint),
                "details": [str(e)],
            },
        }
        if verbose:
...
def _backupProperties(item, verbose=None, outcome=None, outPath=""):
    ...
    try:
        ...
        backupFile = os.path.join(
            tempfile.gettempdir() if not outPath else outPath,
            "{}_Backup.json".format(item.id),
        )
        if os.path.exists(backupFile):
            ...
            try:
                ...
                if verbose:
                    print(
                        " * Failed to load Backup File '{}', error: '{}'".format(
                            backupFile, e
                        )
                    )
                    ...
                    thumbnail = "".join(
                        [
                            "data:image/{};base64,".format(
                                os.path.splitext(item.thumbnail)[-1][1:]
                            )
                        ]
                        + base64.encodebytes(thumbnail).decode().split()
                    )
                    setattr(
                        item,
                        "backupItemThumbnail",
                        {"file_name": item.thumbnail, "encoded_image": thumbnail},
                    )
                except Exception as e:
                    if verbose:
                        print(
                            " * Failed to backup item Thumbnail, error: '{}'".format(e)
                        )

        # Backup Item Data, if it has any!
        for loop in range(2, -1, -1):
            setattr(
                item, "backupItemData", backupDetails.get("itemData", item.get_data())
            )
        ...
        managerProperties = (
            dict(manager.properties) if hasattr(manager, "properties") else {}
        )
        if managerProperties:
            # Service Layer properties differ from Layer properties, which contain more detail
            managerProperties["layers"] = (
                [dict(layer.properties) for layer in manager.layers]
                if hasattr(manager, "layers")
                else []
            )
            managerProperties["tables"] = (
                [dict(table.properties) for table in manager.tables]
                if hasattr(manager, "tables")
                else []
            )
        ...
        if serviceDetails:
            setattr(
                item,
                "backupServiceProperties",
                makeDict(serviceDetails, serviceProperties),
            )
            ...
            relatedItems = (
                [
                    relItem.id
                    for relItem in item.related_items("Service2Service", "reverse")
                ]
                if manager.properties.get("isView", False)
                else []
            )
            setattr(
                item,
                "backupRelationships",
                relatedItems
                if len(relatedItems) == 2
                else backupDetails.get("relatedItems", relatedItems),
            )
            ...
            setattr(
                item,
                "backupLayerProperties",
                eval(serviceDetails.get("layers", []).__repr__()),
            )
            setattr(
                item,
                "backupTableProperties",
                eval(serviceDetails.get("tables", []).__repr__()),
            )
        ...
        if verbose:
            traceback.print_exc()
            print(
                " * Backup Properties Failed, Item: {}, '{}', Error: {}".format(
        ...
        outcome["items"].append(
            {
                ...
                "title": item.title,
                "itemType": item.type,
                "action": "backup service/item properties",
                "result": str(e),
            }
        )
        outcome["success"] = False  # Set as error
...
def _restoreProperties(
    ...
    serviceProperties = (
        item.backupServiceProperties if hasattr(item, "backupServiceProperties") else {}
    )
    itemProperties = (
        item.backupItemProperties if hasattr(item, "backupItemProperties") else {}
    )
    itemThumbnail = (
        item.backupItemThumbnail if hasattr(item, "backupItemThumbnail") else {}
    )
    layerProperties = (
        item.backupLayerProperties if hasattr(item, "backupLayerProperties") else []
    )
    tableProperties = (
        item.backupTableProperties if hasattr(item, "backupTableProperties") else []
    )
    itemData = item.backupItemData if hasattr(item, "backupItemData") else {}
    relatedItems = (
        item.backupRelationships if hasattr(item, "backupRelationships") else []
    )
    ...
    if verbose and manager and not noProps:
        print(
            "\nCurrent Details Gathered, Elapsed Time: {}".format(
                datetime.datetime.now() - startRestore
            )
        )
    ...
    if relatedItems:
        ...
        updateRelationships(
            item, relatedItems, verbose=verbose, outcome=outcome, dryRun=dryRun
        )
    ...
    if not noProps:
        ...
        if hasattr(manager, "properties"):
            serviceIndexes = (
                set()
            )  # Record any indexes created for service, as not to repeat on multiple Layers!
            isView = "isView" in manager.properties and manager.properties["isView"]
            # Restore Layer and Table properties
            for title, array, backup in [
                [
                    "Layer",
                    manager.layers if hasattr(manager, "layers") else [],
                    layerProperties,
                ],
                [
                    "Table",
                    manager.tables if hasattr(manager, "tables") else [],
                    tableProperties,
                ],
            ]:
                for index, layer in enumerate(array):
                    ...
                    userTable = (
                        layer.properties.get("adminLayerInfo", {})
                        .get("tableName", "")
                        .split(".", 1)[-1]
                    )
                    timeDefinition = {}
                    ...
                    if "multiScaleGeometryInfo" in serviceDefinition and not isView:
                        multiScaleGeometry = {
                            "multiScaleGeometryInfo": eval(
                                serviceDefinition["multiScaleGeometryInfo"].__repr__()
                            )
                        }
                        if useDP:
                            multiScaleGeometry["multiScaleGeometryInfo"][
                                "generalizationType"
                            ] = "DP"
                    ...
                    if verbose:
                        isDP = (
                            multiScaleGeometry.get("multiScaleGeometryInfo", {}).get(
                                "generalizationType", ""
                            )
                            == "DP"
                        )
                        print(
                            "\nRestoring {} Properties for: '{}'{}".format(
                                title,
                                layerName,
                                " * Optimized Layer{} *".format(
                                    " (w/DP)" if isDP else ""
                                )
                                if multiScaleGeometry
                                else "",
                            )
                        )
                    ...
                    if "timeInfo" in serviceDefinition:
                        if touchTimeSeries:
                            timeDefinition["timeInfo"] = dict(
                                serviceDefinition.get("timeInfo", {})
                            )
                        ...
                        if not isView:
                            ...
                            for index in serviceDefinition["indexes"]:
                                ...
                                for newLayerIndex in layer.properties["indexes"]:
                                    if (
                                        indexFields
                                        == str(newLayerIndex["fields"]).lower()
                                    ):  # v2.1.2
                                        break
                                else:
                                    skipMessage = ""
                                    ...
                                    if noIndexes:
                                        ...
                                        for field in index["fields"].split(","):
                                            ...
                                            if not field.lower() in fields:
                                                skipMessage = "index field '{}' does not exist!".format(
                                                    field
                                                )
                                                break
                                            elif (
                                                fields[field.lower()].get("length", 0)
                                                > 4000
                                            ):
                                                skipMessage = "index field '{}' cannot be used as a key column, too large (>4000 bytes)!".format(
                                                    field
                                                )
                                                break
                                    ...
                                    if skipMessage:
                                        if verbose:
                                            print(
                                                " * Skipping missing index: '{}', {}".format(
                                                    index["name"], skipMessage
                                                )
                                            )
                                        continue
                                    ...
                                    # Add Index to list
                                    indexName = "_".join(
                                        [userTable] + indexFields + ["idx"]
                                    )
                                    if indexName in serviceIndexes:
                                        if verbose:
                                            print(
                                                " * Skipping creation, index already exists: '{}'".format(
                                                    indexName
                                                )
                                            )
                                        continue
                                    ...
                                    indexes["indexes"][-1]["fields"] = ",".join(
                                        indexFields
                                    )  # If Multi-field index, join using comma

                                    if verbose:
                                        print(
                                            " - Add missing Index for Field(s): '{}', index: '{}'".format(
                                                indexes["indexes"][-1]["fields"],
                    ...
                    for property in list(serviceDefinition.keys()):
                        ...
                        if property not in layer.properties:
                            if verbose:
                                print(
                                    " - Ignoring Layer Property: '{}', no longer on file!".format(
                                        property
                                    )
                                )
                            del serviceDefinition[property]
                    ...
                    if serviceDefinition and verbose:
                        print(
                            " - {} Properties to Restore: '{}'".format(
                                title, "', '".join(serviceDefinition.keys())
                            )
                        )
                    ...
                    for (
                        ...
                        if asyncCall and dataSize > 65535:
                            if verbose:
                                print(
                                    " * Cannot Restore {} properties Asynchronously, size ({:.1f}KB) Exceeds 64KB max, switching to Synchronous!".format(
                                        defTitle, float(dataSize) / 1024
                                    )
                                )
                            asyncCall = False
                            data["async"] = False
                            ...
                            if dryRun:
                                ...
                                if asyncCall:
                                    status = _asyncJob(
                                        ...
                                        verbose=True
                                        if maxVerbose
                                        else (None if verbose else False),
                                        noWait=asyncNoWait,
                                    )
                                else:
                                    # Apply Definition X times, to make sure it sticks!
                                    for loop in range(applyDef - 1, -1, -1):
                                        status = layer._gis._con.post(
                                            layer.url + "/" + defAction, data
                                        )
                                        if loop:
                        ...
                        if not (
                            isinstance(status, dict) and status.get("success", False)
                        ):
                            outcome["items"].append(
                                {
                                    "id": item.id,
                                    "title": item.title,
                                    "itemType": item.type,
                                    "action": "restore layer '{}' properties".format(
                                        layerName
                                    ),
                                    "result": str(status),
                                }
                            ...
                            if verbose:
                                _prints(
                                    "\r * Failed to Restore {} properties: '{}', Elapsed Time: {}".format(
                                        defTitle,
                                        status,
                                        datetime.datetime.now() - start,
                                    ),
                                    status.get("progressLength", 0)
                                    if hasattr(status, "get")
                                    else 0,
                                )
                        elif verbose:
                            _prints(
                                "\r - {} Details Restored! Elapsed Time: {}".format(
                                    defTitle, datetime.datetime.now() - start
                                ),
                                status.get("progressLength", 0)
                                if hasattr(status, "get")
                                else 0,
                ...
                status = _asyncJob(
                    ...
                    "updateDefinition",
                    {
                        ...
                        "updateDefinition": json.dumps(
                            obj=serviceProperties, separators=(",", ":")
                        ),
                    },
                    verbose=True if maxVerbose else (None if verbose else False),
                )
            if not (
                status
                ...
                if verbose:
                    _prints(
                        "\r * Failed to Restore Service Properties, Status: {}".format(
                            status
                        ),
                        status.get("progressLength", 0)
                        if hasattr(status, "get")
                ...
                outcome["items"].append(
                    {
                        ...
                        "title": item.title,
                        "itemType": item.type,
                        "action": "restore service properties",
                        "result": str(status),
                    }
                )
                outcome["success"] = False  # Set as error
                ...
                if changes:
                    _prints(
                        "\r - Properties Restored, Elapsed Time: {}".format(
                            datetime.datetime.now() - start
                        ),
                        status.get("progressLength", 0)
                        if hasattr(status, "get")
                ...
                #
                print(
                    "\nService Properties Restored, Elapsed Time: {}".format(
                        datetime.datetime.now() - startRestore
                    )
                )
        ...
        if (
            json.dumps(item.get_data()) == itemDataStr
        ):  # Compare current data value to backup
        ...
        if itemProperties or itemDataStr:
            ...
            if not (status is True):
                if verbose:
                    print(
                        " * Failed to Restore Item Properties, Status: {}".format(
                            status
                        )
                    )
                outcome["items"].append(
                    {
                        ...
                        "title": item.title,
                        "itemType": item.type,
                        "action": "restore item properties",
                        "result": str(status),
                    }
                )
                outcome["success"] = False  # Set as error
                ...
                print(
                    " - {} Restored, Elapsed Time: {}".format(
                        post, datetime.datetime.now() - start
                    )
                )
                print("\n".join(changes))
        ...
        if (
            itemThumbnail.get("file_name")
            and not itemThumbnail.get("file_name", "") == item_thumbnail
        ):
            status = item.update_thumbnail(**itemThumbnail)
            if not (status is True):
                if verbose:
                    print(
                        " * Failed to Restore Item Thumbnail, Status: {}".format(status)
                    )
                outcome["items"].append(
                    {
                        "id": item.id,
                ...
                print(
                    "    Key: '{}'\n   From: '{}'\n     To: '{}'".format(
                        "Thumbnail", item_thumbnail, itemThumbnail.get("file_name")
                    )
                )
    ...
    if setRelated or (
        relatedItems
        and not len(relatedItems)
        == len(
            [relItem.id for relItem in item.related_items("Service2Service", "reverse")]
        )
    ):
        if verbose:
    ...
    if backupFile:
        if not outcome["success"] is False and not preserveProps:
            if verbose:
                print(
                    " - Dropping Backup{1} File: '{0}'".format(
                        backupFile,
                        ", Property Preservation DISABLED,"
                        if not preserveProps
                        else "",
                    )
                )
            ...
            print(
                " * Retaining Backup{1} File: '{0}'".format(
                    backupFile,
                    ", Property Preservation in effect," if preserveProps else "",
                )
            )
    ...
    if verbose:
        if not noProps:
            print(
                "\n{} Property Restoration! Total Elapsed Time: {}".format(
                    "Successfully Completed"
                    if not outcome["success"] is False
...
def _checkView(
    view, verbose=None, outcome=None, hadError=False, dryRun=False, outPath=""
):
    ...
    addLayersFile = os.path.join(
        tempfile.gettempdir() if not outPath else outPath,
        "{}_addLayers.json".format(view.id),
    )
    relatedItems = (
        view.backupRelationships if hasattr(view, "backupRelationships") else []
    )
    ...
    if os.path.exists(addLayersFile) and not (
        view.layers or viewDetails["layers"] or view.tables or viewDetails["tables"]
    ):
        ...
        if (
            isinstance(addLayers, dict)
            and "layers" in addLayers
            and addLayers["layers"]
        ):
            if verbose:
                ...
                if dryRun:
                    ...
                    data = {
                        ...
                        "addToDefinition": json.dumps(
                            obj=addLayers, separators=(",", ":")
                        ),
                    }
                    status = viewManager._gis._con.post(
                        viewManager.url + "/" + "addToDefinition", data
                    )
    ...
    if relatedItems:
        relatedServices = [
            relItem.id for relItem in view.related_items("Service2Service", "reverse")
        ]
...
def _importConverter(moduleName):
    ...
    for folder in (
        [filePath]
        + [os.path.join(filePath, converterFolder)]
        + [os.path.join(filePath, modPath) for modPath in moduleName.split(".")[:-1]]
    ):
    ...
    try:
        ...
        if not mod.convert.__code__.co_argcount:
            raise Exception(
                "Function 'convert' has NO parameters, minimum of 1 is required"
            )

    except Exception as e:
        raise Exception(
            "Failed to import module '{}', Error: '{}'".format(moduleName, e)
        )
...
def updateRelationships(
    view, relateIds=[], unRelate=False, verbose=None, outcome=None, dryRun=False
):
    ...
    if unRelate and isinstance(unRelate, str):
        ...
        while related:
            for item in related:
                if item in removed:
                    ...
                    if verbose:
                        print(
                            " * Failed to Remove Relationship to Item: {}, '{}'".format(
                                item.id, item.title
                            )
                        )
                    outcome["items"].append(
                        {
                            ...
                            "result": "Failed to Remove Relationship to Item: {}, '{}'".format(
                                item.id, item.title
                            ),
                        }
                    )
                ...
                if verbose:
                    print(
                        " - Removing Relationship to item: {}, '{}'".format(
                            item.id, item.title
                        )
                    )
                ...
                if direction == "reverse":
                    try:
                        if dryRun:
                            ...
                            status = item.delete_relationship(
                                view, unRelate
                            )  # Delete Relationship of View in Item
                    except Exception as e:
                        status = str(e)
                else:
                    try:
                        if dryRun:
                            ...
                            status = view.delete_relationship(
                                item, unRelate
                            )  # Delete Relationship of Item in View
                    except Exception as e:
                        status = str(e)
                ...
                if status is False:
                    if verbose:
                        print(
                            " * Failed to Remove Relationship to Item: {}, '{}', Outcome: {}".format(
                    ...
                    outcome["items"].append(
                        {
                            ...
                            "itemType": view.type,
                            "action": "remove relationship",
                            "result": "Failed to Remove Relationship to Item: {}, '{}', Outcome: {}".format(
                                item.id, item.title, status
                            ),
                        }
                    )
                ...
                if outcome["success"] is None or not status:
        ...
        children = [
            item.id for item in view.related_items("Service2Service", "forward")
        ]  # Create Child Lookup
        parents = [
            item.id for item in view.related_items("Service2Service", "reverse")
        ]  # Create Parent Lookup
        data = [
            item.id for item in view.related_items("Service2Data", "forward")
        ]  # Create Data Lookup
        ...
        for itemId in relateIds:
            item = view._gis.content.get(
                itemId
            )  # Leverage View's arcgis API GIS connection to load Item for Relate operation
            if not item:
                if verbose:
                ...
                outcome["items"].append(
                    {
                        ...
                        "itemType": view.type,
                        "action": "remove relationship",
                        "result": "Failed to find specified Item: {}".format(itemId),
                    }
                )
                outcome["success"] = False
                continue
            ...
            if unRelate:
                if itemId not in related:
                    if verbose:
                        print(
                            " * Ignored * Remove Relationship, View not related to Item: {}, '{}'".format(
                                item.id, item.title
                            )
                        )
                    continue
                ...
                if itemId in children:
                    if verbose:
                        print(
                            " - Removing Child Relationship to Item: {}, '{}'".format(
                                item.id, item.title
                            )
                        )
                ...
                if itemId in parents:
                    if verbose:
                        print(
                            " - Removing Parent Relationship to Item: {}, '{}'".format(
                                item.id, item.title
                            )
                        )
                ...
                addRelationship, addLookup, addDirection = (
                    ("Service2Data", data, "reverse")
                    if item.type in dataItemTypes
                    else ("Service2Service", children, "forward")
                )
                # Add relationship
                if addRelationship == "Service2Data" and (
                    data or item.related_items("Service2Data", "reverse")
                ):
                    if verbose:
                        if data:
                            print(
                                " * Ignored * Add Relationship, Service already Related to a Data Item!"
                            )
                        else:
                            print(
                                " * Ignored * Add Relationship, Data Item already Related to a Service!"
                            )
                elif itemId not in addLookup:
                    if verbose:
                        print(
                            " - Adding Relationship to Item: {}, '{}'".format(
                                item.id, item.title
                            )
                        )
                    ...
                    try:
                        if dryRun:
                            ...
                            status = (
                                item.add_relationship(view, addRelationship)
                                if addDirection == "forward"
                                else view.add_relationship(item, addRelationship)
                            )
                    except Exception as e:
                        status = str(e)
                    ...
                    if status is False:
                        if verbose:
                            print(
                                " * Failed to Add Relationship to Item: {}, '{}', Outcome: {}".format(
                                    item.id, item.title, status
                                )
                            )
                        outcome["items"].append(
                            {
                                ...
                                "result": "Failed to Add Relationship to Item: {}, '{}', Outcome: {}".format(
                                    item.id, item.title, status
                                ),
                            }
                        )
                    ...
                    print(
                        " * Ignored * Add Relationship, View already Related to Item: {}, '{}'".format(
                            item.id, item.title
                        )
                    )
        ...
        if verbose:
            relationships = 0
            dataItems = []
            for index, (itemText, viewText, relationshipType, direction) in enumerate(
                [
                    (
                        "Relies on Data Item",
                        "Relies on Data Item",
                ...
                for item in items:
                    ...
                    print(
                        "{: >22}: {}, '{}' ({}{})".format(
                            viewText if isView else itemText,
                            item.id,
                            item.title,
                            item.type,
                            " View" if isView else "",
...
def getFeatureServiceTarget(
    view, verbose=None, outcome=None, ignoreDataItemCheck=False
):
    ...
    if verbose:  # Display unless told not to
    ...
    if outcome["success"]:
        ...
        #
        if not (
            hasattr(viewManager, "properties")
            and hasattr(viewManager.properties, "isView")
            and viewManager.properties.isView
        ):
            outcome["items"].append(
                {
                    "id": view.id,
                    ...
                    "itemType": view.type,
                    "action": "verify",
                    "result": "Feature Service Item is NOT a 'View'",
                }
            )
            outcome["success"] = False  # Set as error
        else:
            ...
            if not len(views) == 2:
                outcome["items"].append(
                    {
                        ...
                        "itemType": view.type,
                        "action": "verify",
                        "result": "Feature Service View requires 2 Related Feature Services or 2 Related Feature Views, found: {}".format(
                            len(views)
                        ),
                    }
                )
                            ...
                            outcome["items"].append(
                                {
                                    ...
                                    "itemType": relatedView.type,
                                    "action": "verify",
                                    "result": "Related View does NOT contain a Data Source",
                                }
                            )
                            outcome["success"] = False  # Set as error
                            continue
                        ...
                        outcome["items"].append(
                            {
                                ...
                                "action": "verify",
                                "result": "Target Feature Service '{}' is missing a file Data Source".format(
                                    serviceItem.title
                                ),
                            }
                        )
                        ...
                        targets.append(
                            {
                                "view": relatedView,
                                "service": serviceItem,
                                "filename": fileItem[0].name if fileItem else "N/A",
                                "fileType": fileItem[0].type if fileItem else "N/A",
                            }
                        )
                ...
                if not targets:
                    outcome["items"].append(
                        {
                            ...
                            "itemType": view.type,
                            "action": "verify",
                            "result": "No Data Service Target available",
                        }
                    )
                    outcome["success"] = False  # Set as error
                else:
                    currentService = (
                        "* Unknown or NOT Set - Targeting First Available *"
                    )
                    ...
                    try:
                        ...
                        currentService = (
                            "/"
                            + layer.adminLayerInfo.viewLayerDefinition.sourceServiceName
                            + "/FeatureServer"
                        )

                        # Grab Last Edit Date, save to view
                        serviceLastModified = layer.get("editingInfo", {}).get(
                            "lastEditDate", 0
                        )
                        serviceLastModified = (
                            0
                    ...
                    if verbose:
                        print(
                            " -         Current Feature Service: {}".format(
                                currentService
                            )
                        )
                    ...
                    for target in targets:
                        ...
                        if not target["service"].url.endswith(currentService):
                            if verbose:
                                print(
                                    " -  Target Feature Service Item Id: {}".format(
                                        target["service"].id
                                    )
                                )
                                print(
                                    " -    Target Feature Service Title: '{}'".format(
                                        target["service"].title
                                    )
                                )
                                print(
                                    " - Target Feature Service Filename: '{}' ({})".format(
...
def swapFeatureViewLayers(
    ...
    if verbose:
        print(
            "\nInvoking Feature View Layer Swap Workflow{}...".format(
                " (Target update ONLY!)" if noSwap else ""
            )
        )
        ...
        outcome["items"].append(
            {
                ...
                "itemType": view.type,
                "action": "verify",
                "result": "Mutually exclusive parameters are set, cannot Preserve and Ignore Service/View Properties!",
            }
        )
        outcome["success"] = False  # Set as error
            ...
            outcome["items"].append(
                {
                    ...
                    "itemType": item.type,
                    "action": "verify",
                    "result": "Unable to locate specified output path: '{}'!".format(
                        outPath
                    ),
                }
            )
            ...
            outcome["items"].append(
                {
                    ...
                    "itemType": item.type,
                    "action": "verify",
                    "result": "Specified output path: '{}', is NOT a Folder!".format(
                        outPath
                    ),
                }
            )
    ...
    if converter:
        ...
        if isinstance(converter[0], str):
            try:
                ...
                argNames = converter[0].convert.__code__.co_varnames[
                    1:argCount
                ]  # excluding download filename

                if len(converter) > argCount:
                    if argCount > 1:
                        raise Exception(
                            "Too many Parameters specified, {}, only need values for ('{}')".format(
                                len(converter) - 1, "', '".join(argNames)
                            )
                        )
                    else:
                        raise Exception(
                            "No Parameters required, {} specified".format(
                                len(converter) - 1
                            )
                        )
                ...
                for index, value in enumerate(converter[1:]):
                    try:
                        ...
                        raise Exception(
                            "Failed to evaluate Parameter '{}', Error: '{}'".format(
                                argNames[index], e
                            )
                        )
                ...
                outcome["items"].append(
                    {
                        ...
                        "itemType": view.type,
                        "action": "verify",
                        "result": "Loading Converter '{}' Failed, Error: '{}'!".format(
                            converter[0], e
                        ),
                    }
                )
        ...
        outcome["items"].append(
            {
                ...
                "itemType": view.type,
                "action": "verify",
                "result": "Item is NOT a Feature Service 'View', cannot Swap Layers!",
            }
        )
        outcome["success"] = False  # Set as error
            ...
            outcome["items"].append(
                {
                    ...
                    "itemType": item.type,
                    "action": "verify",
                    "result": "Swapping Layers is NOT allowed, a dependent WFS Service exists!",
                }
            )
            outcome["success"] = False  # Set as error
    ...
    layerBackupFile = os.path.join(
        tempfile.gettempdir() if not outPath else outPath,
        "{}_Layers.json".format(view.id),
    )
    addLayersFile = os.path.join(
        tempfile.gettempdir() if not outPath else outPath,
        "{}_addLayers.json".format(view.id),
    ...
    hadError = _checkView(
        view, verbose=verbose, outcome=outcome, dryRun=dryRun, outPath=outPath
    )

    #
    # Get Target Feature Service Details
    #
    target = getFeatureServiceTarget(
        view,
        verbose=verbose,
        outcome=outcome,
        ignoreDataItemCheck=False if updateFile else True,
    )

    if "service" in target:
        viewIsService = (
            target["view"] == target["service"]
        )  # Is the Target View a Service?
        ...
        if updateFile:
            ...
            serviceLastModified = (
                0
                if not hasattr(view, "serviceLastModified")
                else view.serviceLastModified
            )
            setattr(
                target["service"], "returnUpdatedItem", True
            )  # Tell overwriteFeatureService function to return the updated item object instead of the status
            target["service"] = overwriteFeatureService(
                target["service"],
                updateFile=updateFile,
                touchItems=touchItems,
                verbose=verbose,
                touchTimeSeries=touchTimeSeries,
                outcome=outcome,
                ignoreItems=view.id,
                serviceLastModified=serviceLastModified,
                noIndexes=noIndexes,
        ...
        # If Successful Update or no update and no errors
        elif (updateFile and outcome["success"] is True) or not (
            updateFile or outcome["success"] is False
        ):
            if verbose:
            ...
            targetManager = _getManager(
                target["view"], verbose=verbose, outcome=outcome
            )
            ...
            viewLayers = {
                "layers": []
            }  # Updated Layer details for Main View, used to Swap View Layers!
            viewManager = _getManager(view, verbose=verbose, outcome=outcome)
            if outcome["success"] is False:
                return outcome
            ...
            for layer in targetManager.properties.get("layers", []):
                if "viewLayerDefinition" not in layer["adminLayerInfo"]:
                    ...
                    # From Feature View
                    adminId = layer["adminLayerInfo"]["viewLayerDefinition"][
                        "sourceLayerId"
                    ]
                    ...
                    targetLayers[targetId] = {
                        "viewLayerDefinition": {
                            "sourceServiceName": layer["adminLayerInfo"][
                                "viewLayerDefinition"
                            ]["sourceServiceName"],
                            "sourceLayerId": adminId,
                            "sourceLayerFields": layer["adminLayerInfo"][
                                "viewLayerDefinition"
                            ]["sourceLayerFields"],
                        }
                    }
                ...
                for property in ["geometryField", "xssTrustedFields"]:
                    if property in layer["adminLayerInfo"]:
                        targetLayers[targetId][property] = layer["adminLayerInfo"][
                            property
                        ]
            ...
            if not layers:
                ...
                if os.path.exists(layerBackupFile):
                    ...
                    try:
                        ...
                        if verbose:
                            print(
                                " * Note * Failed to load 'Layers' backup file: '{}', Error: '{}'".format(
                                    layerBackupFile, e
                                )
                            )
            ...
            for layer in layers:
                adminId = str(
                    layer["adminLayerInfo"]["viewLayerDefinition"]["sourceLayerId"]
                )
                dropLayers["layers"].append(
                    {"id": layer["id"]}
                )  # Add Layer to drop list
                viewLayers["layers"].append(
                    eval(layer.__repr__())
                )  # Save Layer details
                    ...
                    outcome["items"].append(
                        {
                            ...
                            "itemType": view.type,
                            "action": "swap layers",
                            "result": "Layer id '{}' has no matching source Layer, id '{}', in Target View".format(
                                layer["id"], adminId
                            ),
                        }
                    )
            ...
            if not outcome["success"] is False:
                ...
                if serviceHasLayer:
                    if dryRun:
                        ...
                        data = {
                            "f": "json",
                            "async": asyncCall,
                            "deleteFromDefinition": json.dumps(
                                obj=dropLayers, separators=(",", ":")
                            ),
                        }
                        ...
                        if asyncCall:
                            ...
                            try:
                                status = viewManager._gis._con.post(
                                    viewManager.url + "/" + "deleteFromDefinition", data
                                )
                            except Exception as e:
                                status = e
                                if verbose:
                ...
                if not (
                    ...
                    outcome["items"].append(
                        {
                            ...
                            "title": view.title,
                            "itemType": view.type,
                            "action": "delete from service definition",
                            "result": status,
                        }
                    )
                    outcome["success"] = False  # Set as error
                    ...
                    for applyGroup, applyLayers in [
                        [
                            "Adding Target",
                            addLayers if viewLayers["layers"] else altLayers,
                        ],
                        ["Restoring Original", fallbackLayers],
                    ]:
                        if not applyLayers:
                            continue
                        ...
                        if (
                            ...
                            if verbose:
                                print(
                                    " - View now points to Feature Service: {}, '{}'".format(
                                        target["service"].id, target["service"].title
                                    )
                                )
                                print(
                                    "\nFeature View Layers Swapped, Total Elapsed Time: {}".format(
                                        datetime.datetime.now() - swapStart
                                    )
                                )
                            ...
                            outcome["items"].append(
                                {
                                    ...
                                    "title": view.title,
                                    "itemType": view.type,
                                    "action": "add to service definition",
                                    "result": status,
                                }
                            )
                            hadError = True
                    ...
                    if not (
                        ...
                        try:
                            start = datetime.datetime.now()
                            serviceDetails = viewManager._gis._con.post(
                                viewManager.url, {"f": "json"}
                            )

                            if "layers" in serviceDetails:
                                # Save Layers to backup file
                                json.dump(
                                    serviceDetails["layers"],
                                    open(layerBackupFile, "w"),
                                    indent=3,
                                    separators=(",", ": "),
                                )
                                ...
                                if verbose:
                                    print(
                                        "\nLayer Details Backed up, not related to Preservation, Total Elapsed Time: {}".format(
                                            datetime.datetime.now() - start
                                        )
                                    )
                            ...
                            if verbose:
                                traceback.print_exc()
                                print(
                                    " * Note * Failed to create 'Layers' backup file: '{}', Error: '{}'".format(
                                        layerBackupFile, e
                                    )
                                )
                ...
                if verbose:
                    print(
                        "\nFeature View Layer Swap {} Overall Elapsed Time: {}".format(
                            "* Failed!"
                            if outcome["success"] is False
                            else "was Successful,"
                            if not hadError
                ...
                outcome["success"] = outcome["success"] and not hadError

    return outcome


def overwriteFeatureService(
    item,
    updateFile=None,
    touchItems=True,
    verbose=None,
    ...
    def touchItem(item, message, outcome):
        ...
        try:
            ...
            status = (
                status
                if "error code" not in status.lower()
                else status.replace("\n", " ")
            )
    ...
    def touchTimeInfo(item, message, outcome):
        ...
        try:
                ...
                if definition:
                    if dryRun:
                        ...
                        if asyncCall:
                            ...
                            status = layer._gis._con.post(
                                layer.url + "/" + "updateDefinition", data
                            )
            ...
            if status is None:
                ...
                if dryRun:
                    ...
                    if asyncCall:
                        ...
                        status = layer._gis._con.post(
                            manager.url + "/" + "updateDefinition", data
                        )
            ...
            if verbose:
                traceback.print_exc()
                print(
                    "\n * Failed to Update Service details for Item Id: '{}'{}, Outcome: '{}'".format(
                        item.id,
                        ", Layer: '{}'".format(layerName) if layerName else "",
                        e,
                    )
                )
            status = "Failed, Outcome: '{}'".format(e)
    ...
    isFileItem = (item.type in fileItemTypes) or (
        item.name and "." in item.name
    )  # is allowed type or is a filename in item?

    if not outcome:
        outcome = {"success": None, "items": []}

    if verbose and updateFile:
        print(
            "\nInvoking Overwrite {}...".format(
                "File Item" if isFileItem else "Feature Service"
            )
        )
    ...
    returnItem = (
        hasattr(item, "returnUpdatedItem") and item.returnUpdatedItem
    )  # Used by Sizzle logic!
    ...
    if not (item.type == "Feature Service" or isFileItem):
        outcome["items"].append(
            {
                ...
                "itemType": item.type,
                "action": "verify",
                "result": "Item Type is NOT a 'Feature Service' or supported File Item Type",
            }
        )
        outcome["success"] = False  # Set as error
    else:
        ...
        #
        manager = (
            _getManager(item, verbose=verbose, outcome=outcome)
            if not isFileItem
            else None
        )
        if outcome["success"] is False:
        ...
        if "View Service" in item.typeKeywords:
                ...
                outcome["items"].append(
                    {
                        ...
                        "itemType": item.type,
                        "action": "verify",
                        "result": "Feature Service Item is a 'View', cannot Overwrite",
                    }
                )
                outcome["success"] = False  # Set as error
                return outcome
            ...
            # Check status of View's Layers
            _checkView(
                item, verbose=verbose, outcome=outcome, dryRun=dryRun, outPath=outPath
            )
        ...
        if updateFile:
            ...
            #
            hasChangeTrackingOnView = (
                manager.properties.get("hasChangeTrackingEnabledViews", False)
                if not isFileItem
                else False
            )
                ...
                outcome["items"].append(
                    {
                        ...
                        "itemType": item.type,
                        "action": "verify",
                        "result": "Overwrite on Service is NOT allowed, a dependent View or Service has Change Tracking Enabled!",
                    }
                )
                outcome["success"] = False  # Set as error
                    ...
                    outcome["items"].append(
                        {
                            ...
                            "itemType": item.type,
                            "action": "verify",
                            "result": "Overwrite on Service is NOT allowed, a dependent WFS Service exists!",
                        }
                    )
                    outcome["success"] = False  # Set as error
            ...
            if manager:
                for layer in manager.layers:
                    if "multiScaleGeometryInfo" in layer.properties:
                        ...
                        if verbose:
                            print(
                                " - Detected Optimization on Layer Id: {}".format(
                                    layer.properties.get("id", "")
                                )
                            )
            ...
            if updateFile.split(":")[0].lower() in ["ftp", "http", "https"]:
                outputFile = os.path.join(
                    tempfile.gettempdir() if not outPath else outPath, outputFile
                )
                ...
                try:
                    ...
                    headers = {
                        "Accept": "*/*",
                        "Accept-Encoding": "*",
                        "User-Agent": "Python/v{} OverwriteFS.py/{}".format(
                            platform.python_version(), version
                        ),
                    }

                    request = urllib.request.urlopen(
                        urllib.request.Request(updateFile, headers=headers)
                    )
                    headers = (
                    ...
                    if verbose:
                        ...
                        print(
                            " - Last Modified: {} (Local Download)".format(
                        ...
                        print(
                            " - Last Modified: {} (URL Content)".format(
                        ...
                        if (
                            ...
                            if verbose:
                                print(
                                    "\n * {}{}!".format(
                                        status,
                                        " * instructed to Ignore" if ignoreAge else "",
                                    )
                                )
                            ...
                            if not ignoreAge:
                                if touchItems:
                                    # for view in item.related_items( "Service2Service"):
                                    for view in item.related_items(
                                        "Service2Data", "reverse"
                                    ):  # Select only Views that rely on this Service
                                        touchItem(
                                            view,
                                            "\nTouching details on related View: '{}'".format(
                                                view.title
                                            ),
                                            outcome,
                                        )
                            ...
                            lastFile = {
                                "filename": outputFile,
                                "CRC": crcValue,
                                "filesize": os.stat(outputFile).st_size,
                            }
                            if maxVerbose:
                                print(
                                    "\nElapsed Time to Calc CRC value on existing file: {}, Value: {}".format(
                                        datetime.datetime.now() - crcStart, crcValue
                                    )
                                )
                    ...
                    outcome["items"].append(
                        {
                            ...
                            "itemType": item.type,
                            "action": "verify",
                            "result": "Update Filename '{}', Originally Published with {}, CANNOT be found in Folder: '{}'".format(
                                outputFile,
                                "Item" if isFileItem else "Service",
                                os.path.split(updateFile)[0],
                            ),
                        }
                    )
                    outcome["success"] = False  # Set as error
                ...
                outcome["items"].append(
                    {
                        ...
                        "itemType": item.type,
                        "action": "verify",
                        "result": "Update Filename '{}' does NOT Exist".format(
                            updateFile
                        ),
                    }
                )
                ...
                outcome["items"].append(
                    {
                        ...
                        "itemType": item.type,
                        "action": "verify",
                        "result": "Update Filename '{}' is NOT a File".format(
                            updateFile
                        ),
                    }
                )
            ...
            if converter:
                ...
                fileTimestamp = os.stat(
                    updateFile
                ).st_mtime  # Save current File Timestamp, in case we need to re-apply
                updatePath, updateFilename = os.path.split(
                    os.path.realpath(outputFile)
                )  # updateFile))  # Extract Path and Filename

                # Run Conversion logic
                try:
                    if verbose:
                        print(
                            "\n - Converting Data using Module: '{}'{}".format(
                                converter.__name__,
                                ", Version: '{}'".format(converter.__version__)
                                if hasattr(converter, "__version__")
                                else "",
                            )
                        )

                    argCount = converter.convert.__code__.co_argcount
                    argNames = converter.convert.__code__.co_varnames[
                        1:argCount
                    ]  # excluding download filename

                    if "verbose" in argNames and (argNames.index("verbose") + 1) > len(
                        args
                    ):  # If 'verbose' available and not in arguments provided, add it!
                        keyargs["verbose"] = True if verbose else False

                    if "checkPublication" in argNames and (
                        argNames.index("checkPublication") + 1
                    ) > len(
                        args
                    ):  # If 'checkPublication' available and not in arguments provided, add it!
                        if ignoreAge:
                    ...
                    outcome["items"].append(
                        {
                            ...
                            "itemType": item.type,
                            "action": "converter",
                            "result": "Data Conversion Failed, Error: '{}'!".format(e),
                        }
                    )
                    outcome["success"] = False  # Set as error
                ...
                outcome["items"].append(
                    {
                        ...
                        "itemType": item.type,
                        "action": "verify",
                        "result": "Update Filename '{}' does NOT match Original Filename used to Publish {}: '{}'".format(
                            updateFile, "Item" if isFileItem else "Service", outputFile
                        ),
                    }
                )
                outcome["success"] = False  # Set as error
            ...
            if lastFile:
                if os.stat(updateFile).st_size == lastFile["filesize"]:
                    ...
                    if maxVerbose:
                        print(
                            "\nElapsed Time to Calc CRC value on file download: {}, Value: {}".format(
                                datetime.datetime.now() - crcStart, crcValue
                            )
                        )
            ...
            #
            # Verify file size does not exceed 2^31 - 1, removed v2.1.3
            #
            # if os.stat( updateFile).st_size >= pow(2, 31) and not globals().get( "nolimit", False):
            #    if verbose:
            ...
            if isFileItem:
                ...
                outcome["success"] = status is not False and not str(status).startswith("Failed")
            ...
            #
            layerProperties = (
                item.backupLayerProperties
                if hasattr(item, "backupLayerProperties")
                else []
            )
            tableProperties = (
                item.backupTableProperties
                if hasattr(item, "backupTableProperties")
                else []
            )
            dropLayers = {"layers": []}
            ...
            try:
                    ...
                    try:
                        if dryRun:
                            ...
                            if (
                                createInfo
                                and dataItem
                                and not dataItem.type == "Service Definition"
                            ):
                                ...
                                try:
                                    ...
                                    tempFile = os.path.sep.join(
                                        [
                                            tempfile.gettempdir(),
                                            "{}_itemInfo.json".format(item.id),
                                        ]
                                    )
                                    updateInfo = "/".join(
                                        [
                                    ...
                                    infoData = {
                                        "name": manager.properties["adminServiceInfo"][
                                            "name"
                                        ],
                                        "maxRecordCount": manager.properties[
                                            "maxRecordCount"
                                        ],
                                        "hasStaticData": manager.properties[
                                            "hasStaticData"
                                        ],
                                        "layerInfo": {
                                            "capabilities": manager.properties[
                                                "capabilities"
                                            ]
                                        },
                                    }
                                    json.dump(infoData, open(tempFile, "w"))
                                    status = item._gis._con.post(
                                        updateInfo,
                                        files=[
                                            ["file", tempFile, "publishParameters.json"]
                                        ],
                                    )
                                    if not (
                                        status
                                        and isinstance(status, dict)
                                        and status.get("success", False)
                                    ):
                                        raise Exception(status)
                                    ...
                                    if verbose:
                                        traceback.print_exc()
                                        print(
                                            " * Failed to Generate Item Info File for Id: '{}', Outcome: '{}' * Skipping!".format(
                                                item.id, e
                                            )
                                        )
                            ...
                            if dataItem and dataItem.type == "GeoPackage":
                                ...
                                if verbose:
                                    print(
                                        " - Updating File Item, Type: '{}'...".format(
                                            dataItem.type
                                        )
                                    )
                                ...
                                if status:
                                    ...
                                    status = dataItem.publish(
                                        overwrite=True, file_type=dataItem.type
                                    )
                                    if isinstance(status, arcgis.gis.Item):
                                        status = {"success": True}
                                else:
                                    raise Exception(
                                        "data item update failed '{}'".format(status)
                                    )
                            else:
                        ...
                        postText = ""
                        if (
                            "item info file does not exist" in str(e).lower()
                            and createInfo is not None
                        ):
                            createInfo = True
                        ...
                        if (
                            "in _add_by_part" in trace
                            and "can only concatenate" in trace
                        ):
                            ...
                            if "job failed" in str(e).lower():
                                ...
                                try:
                                    ...
                                    if asyncCall:
                                        ...
                                        delStatus = manager._gis._con.post(
                                            manager.url + "/" + "deleteFromDefinition",
                                            data,
                                        )
                ...
                if not (
                    status
                    and isinstance(status, dict)
                    and status.get("jobInfo", {}).get("jobType", "") == "publish"
                    and status.get("status", "") == "completed"
                ):
                    raise Exception(
                        "Feature Service 'publish' action did NOT complete as expected, Job Details: '{}'".format(
                            status
                        )
                    )
                ...
                if wasOptimized and not noProps:
                    try:
                        ...
                        wait = (
                            globals().get("deoptimizewait")
                            if globals().get("deoptimizewait")
                            else 60
                        )  # Seconds to wait before attempting to cancel Auto-Optimize!
                        timeout = 120
                        for layer in curDetails["layers"]:
                            if (
                                layer["geometryType"]
                                in ["esriGeometryPolyline", "esriGeometryPolygon"]
                                and "multiScaleGeometryStatus" in layer
                            ):
                                while wait:
                                    sys.stdout.write(
                                        "   - Canceling Layer Optimization in {} second{}... \r".format(
                                            wait, "s" if wait > 1 else ""
                                        )
                                    )
                                    time.sleep(1)
                                    wait -= 1
                                ...
                                for layer in manager.layers:
                                    if layer.properties.get("id", "") == layerId:
                                        ...
                                        if verbose:
                                            print(
                                                "   - Canceling Automatic Optimization on Layer Id: {}".format(
                                                    layer.properties.get("id", "")
                                                )
                                            )
                                        start = datetime.datetime.now()
                                            ...
                                            while retries:
                                                status = _asyncJob(
                                                    ...
                                                    "updateDefinition",
                                                    {
                                                        "f": "json",
                                                        "async": True,
                                                        "updateDefinition": '{"multiScaleGeometryInfo":null}',
                                                    },
                                                    verbose=True
                                                    if maxVerbose
                                                    else (None if verbose else False),
                                        ...
                                        if not (
                                            ...
                                            _prints(
                                                "\r   - Optimization Canceled! Elapsed Time: {}    ".format(
                                                    datetime.datetime.now() - start
                                                ),
                                                status.get("progressLength", 0)
                                                if hasattr(status, "get")
                        ...
                        if verbose:
                            print(
                                "\n * Failed to Check/Cancel Optimization, Error: {}".format(
                                    e
                                )
                            )
    ...
    #
    if (touchItems or touchTimeSeries) and outcome["success"] is True:
        # for view in item.related_items( "Service2Service"):
        for view in item.related_items(
            "Service2Data", "reverse"
        ):  # Select only Views that rely on this Service
            ...
            if touchItems:
                touchItem(
                    view,
                    "\nTouching details on related View: '{}'".format(view.title),
                    outcome,
                )
                ...
                touchTimeInfo(
                    view,
                    "\nTouching Time Series Layers on related View: '{}'".format(
                        view.title
                    ),
                    outcome,
                )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4 or help:
        ...
        print(
            "                            Optimization to complete before continuing the update workflow. When enabled, function will"
        )
        print(
            "                            report condition and supply a URL that can be used for manual status review."
        )
        print(
            "                            Default: Function will wait for properties like Layer Optimization to be re-applied before"
        )
        print(
            "                                     proceeding to the next processing 'step' in the update workflow."
        )
        print(
        ...
        print(
            "                            Default: Cancel Service update if age of <url> data is not newer than last Service update."
        )
        print(
            "\n    -SwapLayers: (optional) Action Switch instructing function to Swap Layers in View, point all Layers to Target"
        )
        print(
            "                            or newly updated Feature Service. Used by A/B Feature Service enabled View, whereby the"
        )
        print(
            "                            View is Related to Two Feature Services, allowing the View's Layers to be swapped out,"
        )
        print(
            "                            pointing them to the matching Layers of the newly updated Feature Service."
        )
        print(
        ...
        print(
            "                            and File Item upload Filename that should be used for the next update target. Leveraged by"
        )
        print(
            "                            A/B View enabled Services to select inactive Feature Service that should be updated next."
        )
        print(
        ...
        print(
            "                            This invokes the -SwapLayers workflow, but only updates the Target Service, allowing"
        )
        print(
            "                            manual QA/QC before a Production Layer Swap."
        )
        print(
            "                            Default: Overwrite action when <filename> specified and no other Action Switches included."
        )
        ...
        print(
            "                            transform the source data before updating the Hosted Feature Service."
        )
        print(
    ...
    validSwitches = [
        ...
        [
            "DeOptimizeWait",
            0,
        ],  # , ["NoLimit", True] # Hidden Parameters, accessible as lower case key in Globals!
    ]
    lowerSwitches = {}
    localVars = locals()
    ...
    for index, arg in enumerate(sys.argv[4:]):
        ...
        hasDash = arg.startswith(
            "-"
        )  # or True # Staged to allow Dashed and Non-Dashed (as a transition) Option Switches
        if lowerArg in lowerSwitches and hasDash:
            ...
            try:
                ...
                exit(
                    "\n\a * Integer Switch '{}' is invalid, bad value '{}'!".format(
                        sys.argv[4:][index - 1], arg
                    )
                )
        else:
            if os.path.exists(arg):
                ...
                if os.path.isfile(arg):
                    # Is a File
                    updateExists = True
                    fileLastModified = datetime.datetime.utcfromtimestamp(
                        int(os.stat(updateFile).st_mtime)
                    )
                    updateStatus = ", Last Modified: {}".format(
                        fileLastModified.strftime("%a, %d %b %Y %H:%M:%S GMT")
                    )
                else:
                    # Is a Directory
    ...
    print(
        "          Switches: {}".format(
            ", ".join(
                [
                    "{}={}".format(
                        key, "*****" if key == "Password" else localVars[key.lower()]
                    )
                    for key, init in validSwitches
                    if not localVars[key.lower()] is None
                ]
            )
        )
    )
    ...
    if updateFile and updateExists is False:
        exit(
            "\n\a * ERROR * Unable to locate Update File or Folder '{}' *".format(
                updateFile
            )
        )
    ...
    verbose = (
        "Max"
        if localVars["moredetail"]
        else (None if localVars["lessdetail"] else True)
    )
    ...
    if not usingPro:
        ...
        print(
            " - Logged in with user: '{} ({})'".format(
                gis.users.me.username, gis.users.me.fullName
            )
        )
    ...
    if gettarget:
        # Get Target Feature Service Details
        getFeatureServiceTarget(
            item, outcome=outcome, verbose=verbose, ignoreDataItemCheck=True
        )
        ...
        swapFeatureViewLayers(
            ...
            updateFile=updateFile,
            verbose=verbose,
            touchTimeSeries=not notimeseries,
            outcome=outcome,
            noIndexes=noindexes,
        ...
        print(
            "\nElapsed Time for {} Process: {}".format(
                "Update Target" if updatetarget else "Swap Layers",
                datetime.datetime.now() - start,
            )
        )
        ...
        listrelated = (
            listrelated
            or (isinstance(addrelated, list) and not addrelated)
            or (isinstance(removerelated, list) and not removerelated)
        )
        ...
        if addrelated and outcome["success"]:
            # Add related Views
            updateRelationships(
                item,
                relateIds=addrelated,
                verbose=verbose,
                outcome=outcome,
                dryRun=dryrun,
            )
        ...
        # Do Overwrite
        overwriteFeatureService(
            item,
            updateFile=updateFile,
            touchItems=not notouch,
            verbose=verbose,
            touchTimeSeries=not notimeseries,
            outcome=outcome,
            noIndexes=noindexes,
        ...
        print(
            "\nElapsed Time for Overwrite Process: {}".format(
                datetime.datetime.now() - start
            )
        )
