...
import ahd_data_pipelines.tasks.esri.layer_definitions.dashboardData as dD
...
from arcgis.features import FeatureLayer, GeoAccessor, FeatureSet, FeatureCollection
...
class GreenDataUpdate(Pipeline):
    def __init__(self, dbutils=None, conf=None, stage="DEV", development=False, log4j_logger=None):
        ...
        self.conf = {
            ...
            "target_update_layer_A": "930d57d1801044818f087395ba0e5bf4",
            "target_update_view": "a8a6a24a628a44d28e960c258ebb2fae",
            "target_update_layer_B": "880ffdf6b84d44cea87799e4f91a8a49",
    ...
    def run(self):
        ...
        type_dict = {"esriFieldTypeInteger": int, "esriFieldTypeString": str, "esriFieldTypeDouble": float, "esriFieldTypeDate": str, "esriFieldTypeSmallInteger": int, "esriFieldTypeSingle": float}
        ...
        def getGeoDataFrameWithSchema(schemaFile, dataDictionary, sdf_dict, layerName):
            pathOpen = os.path.join(os.getcwd(), schemaFile, dataDictionary)
            with open(pathOpen, "r") as pathFile:
                pathData = json.load(pathFile)
            pathFields = pathData["fields"]
            fieldNames = []
            fieldTypes = {}
            for pathField in pathFields:
                if "name" in pathField and pathField["name"] in sdf_dict and pathField["name"] != "OBJECTID".casefold():
                    fieldNames.append(pathField["name"])
                    fieldTypes[pathField["name"]] = type_dict[pathField["type"]]

            gdf = gpd.GeoDataFrame(columns=fieldNames)
            for column, dtype in fieldTypes.items():
                gdf[column] = gdf[column].astype(dtype)

            # Populate the GeoDataFrame using the dictionary
            for key, value in sdf_dict.items():
                gdf[key] = value
            
            if layerName == 'PSPS_Points':
                if PSPS_Points_df.size > 0:
                    gdf = gpd.GeoDataFrame(gdf)
                    gdf.to_file(filename=target_filepath, driver='GPKG', layer=layerName)
                else:
                    def_point = Point(5, 5)
                    PSPS_Points_test = gpd.GeoDataFrame(
                        {
                            'Outage_ID': ["outage id"],
                            'County': ["county"],
                            'Status': ["status"],
                            'Essential_Customers': [1],
                            'Medical_Baseline_Customers': [2],
                            'Circuit': ["circuit"],
                            'Life_Support_Customers': [3],
                            'Utility_Company': ["utility company"],
                            'Planned_Area_ID': ["planned area id"],
                            'Total_Customers': [4],
                            'Major_Customers': [5],
                            'GlobalID': [6],
                    ...
                    PSPS_Points_test.to_file(filename=target_filepath, driver='GPKG', layer=layerName)
            else:
                if "SHAPE" in gdf.columns and gdf["SHAPE"].dtype == "object":
        ...
        #     result_df = remove_columns(result_df, [data_layers[layer_facilities]["object_id_field_name"]])
            ...
            result_df.drop(['ObjectId'], axis=1, inplace=True)
            facilities_sdf_dic = result_df.to_dict('list')

            getGeoDataFrameWithSchema("ade_recurring_data_pipelines\\layer_definitions\\dashboardData", result_layers[layer_facilities]["schemaFile"], facilities_sdf_dic, layer_facilities)
        ...
        try: 
            ...
            if events_df.size > 0:
                events_df["geometry"] = events_df["SHAPE"].apply(lambda x: x.as_shapely if x is not None else None)
            population = calculated[layer_population]["data"].copy()
            all_events_pop_join = Calculate_AllEventsPopJoin(events_df, population)

            if events_df.empty == True or population_df.empty == True:
                AllEventsWithPopulation_df = events_df
            else:
                try:
                    ''' Join and sum data '''
                    events_df['objectid_join'] = events_df.index
                    events_pop_join_df = local_join_df(events_df, population, "left")
                    sum_sdf = events_pop_join_df.groupby('objectid_join')["L0_over55","L0_over65","L0_over75","L0_over85","L0_tot_zip_pop","L0_cdss_calfresh_recpts","L0_cdss_ihss_recpts","L0_cdph_wic_partipts","L0_cdph_medicare_benes","L0_cdph_medicare_dme","L0_dhcs_medi_cal_benes","L0_dhcs_medi_cal_dme","L0_cdds_clients"].sum()
                    AllEventsWithPopulation_df = pd.merge(events_df, sum_sdf, on=['objectid_join'], how='left')

                    AllEventsWithPopulation_df= remove_columns(result_df, remove)
                    AllEventsWithPopulation_df = result_df.rename(columns=field_map)
            ...
            calculated[layer_events_pop_join] = AllEventsWithPopulation_df

            if AllEventsWithPopulation_df.size > 0: 
                AllEventsWithPopulation_df["geometry"] = AllEventsWithPopulation_df["SHAPE"].apply(lambda x: x.as_shapely if x is not None else None)
            AllEventsWithPopulation_sdf_dic = AllEventsWithPopulation_df.to_dict('list')

            getGeoDataFrameWithSchema("ade_recurring_data_pipelines\\layer_definitions\\dashboardData", result_layers[layer_events_pop_join]["schemaFile"], AllEventsWithPopulation_sdf_dic, layer_events_pop_join)
        ...
        try: 
            ...
            if all_events_df.empty == True:
                FacilitiesAllEventsJoin_df = calculated[layer_facilities]["data"]

            else:
                facilities_df = calculated[layer_facilities]["data"]
                all_events_df_copy = all_events_df.copy()
                FacilitiesAllEventsJoin_df = local_join_df(left_df= facilities_df, right_df=all_events_df_copy, join_type="left")
                FacilitiesAllEventsJoin_oid = result_fields[layer_facilities_events_join]["object_id_field_name"]

                FacilitiesAllEventsJoin_df = remove_columns(FacilitiesAllEventsJoin_df, ['OBJECTID_left', 'OBJECTID_right'])
                FacilitiesAllEventsJoin_df.index = range(0,len(FacilitiesAllEventsJoin_df))
                FacilitiesAllEventsJoin_df[FacilitiesAllEventsJoin_oid] = FacilitiesAllEventsJoin_df.index
                FacilitiesAllEventsJoin_df['ROW'] = FacilitiesAllEventsJoin_df.index
                FacilitiesAllEventsJoin_df['Join_Count'] = 1


            calculated[layer_facilities_events_join]["data"] = FacilitiesAllEventsJoin_df

            if FacilitiesAllEventsJoin_df.size > 0: 
                FacilitiesAllEventsJoin_df["geometry"] = FacilitiesAllEventsJoin_df["SHAPE"].apply(lambda x: x.as_shapely if x is not None else None)
            FacilitiesAllEventsJoin_sdf_dic = FacilitiesAllEventsJoin_df.to_dict('list')

            getGeoDataFrameWithSchema("ade_recurring_data_pipelines\\layer_definitions\\dashboardData", result_layers[layer_facilities_events_join]["schemaFile"], FacilitiesAllEventsJoin_sdf_dic, layer_facilities_events_join)
        ...
        try: 
            ...
            if all_events_df.empty: 
                CA_CountiesAllEventsJoin_df = all_events_df
            else: 
                all_events_df_copy = all_events_df.copy()
                CA_CountiesAllEventsJoin_df = local_join_df(left_df=counties_df, right_df=all_events_df_copy, join_type="inner")
                CA_CountiesAllEventsJoin_oid = result_fields[layer_counties_events_join]["object_id_field_name"]

                CA_CountiesAllEventsJoin_df = remove_columns(CA_CountiesAllEventsJoin_df, ['OBJECTID_1', 'objectid_12'])
                CA_CountiesAllEventsJoin_df.index = range(0,len(CA_CountiesAllEventsJoin_df))
                CA_CountiesAllEventsJoin_df[CA_CountiesAllEventsJoin_oid] = CA_CountiesAllEventsJoin_df.index
                CA_CountiesAllEventsJoin_df['ROW'] = CA_CountiesAllEventsJoin_df.index

            calculated[layer_counties_events_join]["data"] = CA_CountiesAllEventsJoin_df

            if CA_CountiesAllEventsJoin_df.size > 0: 
                CA_CountiesAllEventsJoin_df["geometry"] = CA_CountiesAllEventsJoin_df["SHAPE"].apply(lambda x: x.as_shapely if x is not None else None)
            CA_CountiesAllEventsJoin_sdf_dic = CA_CountiesAllEventsJoin_df.to_dict('list')

            getGeoDataFrameWithSchema("ade_recurring_data_pipelines\\layer_definitions\\dashboardData", result_layers[layer_counties_events_join]["schemaFile"], CA_CountiesAllEventsJoin_sdf_dic, layer_counties_events_join)
        ...
        try:
            ...
            if all_events_df.empty: 
                AllEventsCA_CountiesJoinPoints_df = all_events_df
                AllEventsCA_CountiesJoinPoints_sdf_dic = AllEventsCA_CountiesJoinPoints_df.to_dict('list')
            else: 
                counties_df = data_layers[layer_counties]["df"].copy()
                all_events_df_copy = all_events_df.copy()

                AllEventsCA_CountiesJoinPoints_df = local_join_df(left_df=all_events_df_copy, right_df=counties_df, join_type="left")
                AllEventsCA_CountiesJoinPoints_oid = result_fields[layer_events_counties_join_points]["object_id_field_name"]

                # Convert geometry
                sr = AllEventsCA_CountiesJoinPoints_df.spatial.sr
                if AllEventsCA_CountiesJoinPoints_df.size > 0: 
                    AllEventsCA_CountiesJoinPoints_df["geometry"] = AllEventsCA_CountiesJoinPoints_df["SHAPE"].apply(lambda geometry: geometry.as_shapely.centroid)

                    remove = ['shape_leng', 'Shape_Length_1', 'Shape_Area_1', 'objectid_12', 'objectid', 'OBJECTID_1', 'OBJECTID_left', 'OBJECTID_right']
                    AllEventsCA_CountiesJoinPoints_df = remove_columns(AllEventsCA_CountiesJoinPoints_df, remove)
                    AllEventsCA_CountiesJoinPoints_df.index = range(0,len(AllEventsCA_CountiesJoinPoints_df))
                    AllEventsCA_CountiesJoinPoints_df[AllEventsCA_CountiesJoinPoints_oid] = AllEventsCA_CountiesJoinPoints_df.index
                    AllEventsCA_CountiesJoinPoints_df['ROW'] = AllEventsCA_CountiesJoinPoints_df.index

                    calculated[layer_events_counties_join_points]["data"] = AllEventsCA_CountiesJoinPoints_df

                    AllEventsCA_CountiesJoinPoints_sdf_dic = AllEventsCA_CountiesJoinPoints_df.to_dict('list')

            getGeoDataFrameWithSchema("ade_recurring_data_pipelines\\layer_definitions\\dashboardData", result_layers[layer_events_counties_join_points]["schemaFile"], AllEventsCA_CountiesJoinPoints_sdf_dic, layer_events_counties_join_points)
        ...
        try: 
            ...
            PSPS_Points_df = data_layers[layer_psps]["df"].copy()

            if PSPS_Points_df.size > 0: 
                sr = PSPS_Points_df.spatial.sr
                PSPS_Points_df["SHAPE"] = PSPS_Points_df["SHAPE"].apply(lambda geometry: Point({"x" :  geometry.centroid[0], "y" :  geometry.centroid[1], "spatialReference": sr}))
            else:
                PSPS_Points_df["SHAPE"] = None

            calculated[layer_psps_points]["data"] = PSPS_Points_df

            PSPS_Points_df["geometry"] = PSPS_Points_df["SHAPE"].apply(lambda x: x.as_shapely if x is not None else None)
            PSPS_Points_sdf_dic = PSPS_Points_df.to_dict('list')

            getGeoDataFrameWithSchema("ade_recurring_data_pipelines\\layer_definitions\\dashboardData", result_layers[layer_psps_points]["schemaFile"], PSPS_Points_sdf_dic, layer_psps_points)
        ...
        try: 
            ...
            if pop_score_df.size > 0: 
                pop_score_df["geometry"] = pop_score_df["SHAPE"].apply(lambda x: x.as_shapely if x is not None else None)
            pop_score_sdf_dic = pop_score_df.to_dict('list')

            getGeoDataFrameWithSchema("ade_recurring_data_pipelines\\layer_definitions\\dashboardData", result_layers[layer_population_score]["schemaFile"], pop_score_sdf_dic, layer_population_score)
        ...
        try:
            Population_df = calculated[layer_population]["data"].copy()
            Population_county_df = calculated[layer_population]["data"].copy().drop_duplicates(subset='L0_county', keep="first")
            print("Appending {} dummy records to {} with {} records.".format(len(Population_county_df.index), layer_population, len(Population_df.index)))
            ...
            for idx, val in enumerate(Population_county_df.itertuples(index=False)):
                dup_row = list(val)
                columns = Population_county_df.iloc[[idx]].columns
            ...
            Population_dummy_df = pd.DataFrame(temp_series, columns=Population_county_df.columns)
            Population_df = pop_df.append(Population_dummy_df, ignore_index=True)
            Population_df.drop(columns=['FID'], inplace=True)

            print("Records appended. Result record count: {}.".format(len(Population_df.index)))
            calculated[layer_population] = Population_df

            if Population_df.size > 0: 
                Population_df["geometry"] = Population_df["SHAPE"].apply(lambda x: x.as_shapely if x is not None else None)
            Population_sdf_dic = Population_df.to_dict('list')

            getGeoDataFrameWithSchema("ade_recurring_data_pipelines\\layer_definitions\\dashboardData", result_layers[layer_population]["schemaFile"], Population_sdf_dic, layer_population)
