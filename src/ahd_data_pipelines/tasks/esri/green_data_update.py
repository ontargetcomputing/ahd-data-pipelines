...
class GreenDataUpdate(Pipeline):
    ...
    def run(self):
        ...
        def create_jira_ticket(
            ...
            subject="AHD Task Error: Error in DashboardData Update Notebook.",
            message="Error in automated task.",
            ...
            notify_data = {
                "fields": {
                    "assignee": {"accountId": jira_assignee},
            ...
            if response.reason == "Created":
                ...
                comment_url = "{0}{1}{2}".format(
                    jira_config["comment_url"][0],
                    issue_id,
                    jira_config["comment_url"][1],
                )
                comment_headers = {
                ...
                comment_data = {
                    "body": {
                        "type": "doc",
                        "version": 1,
                        "content": [{"type": "paragraph", "content": tag_list}],
                    }
                }
        ...
        population_types_list = [
            "L0_over55",
            "L0_over65",
            "L0_over75",
            "L0_over85",
            "L0_cdss_calfresh_recpts",
            "L0_cdss_ihss_recpts",
            "L0_cdph_wic_partipts",
            "L0_dhcs_medi_cal_benes",
            "L0_dhcs_medi_cal_dme",
            "L0_cdds_clients",
        ]  # TODO: handle 0 values in data "L0_cdph_medicare_benes", "L0_cdph_medicare_dme"
        ...
        def get_features(data):
            ...
            f_set = f_layer.query(
                where=data["where"],
                out_fields=data["out_fields"],
                return_geometry=data["return_geometry"],
                out_sr=out_sr,
            )
                    ...
                    for geometry in data["df"]["SHAPE"]:
                        try:
                            if (
                                geometry == "NaN"
                                or geometry == "nan"
                                or type(geometry) is float
                                or geometry.type.lower() != geo_type
                            ):
                                remove.append(index)
        ...
        def spatial_join_features(
            ...
            if drop_dups is True and record_id is not None:
        ...
        for key, data in data_layers.items():
            ...
            try:
                ...
                try:
                    ...
                    Log_Errors.append(
                        {
                            "message": "Error collecting data for layer {0}. {1}. 504 error recovery method failed.".format(
                                data["layerName"], e
                            )
                        }
                    )
        ...
        if no_errors(Log_Errors):
            ...
            try:
                ...
                all_events_df = calculate_events_data(
                    psps_df=psps_data,
                    fire_df=fire_data,
                    earthquake_df=earthquake_data,
                    evacuation_df=evacuation_data,
                    oid=all_events_oid,
                )
            except Exception as e:
        ...
        if no_errors(Log_Errors):
            try:
                ...
                result_df = calculate_fire_impacted(
                    source_df=result_df,
                    fire_df=data_layers[Events_Type.fire.value]["df"],
                    oid=result_oid,
                )
                result_df = calculate_psps_impacted(
                    source_df=result_df,
                    psps_df=data_layers[Events_Type.psps.value]["df"],
                    group_by_key="ROW",
                )
                result_df = calculate_earthquake_impacted(
                    source_df=result_df,
                    earthquake_df=data_layers[Events_Type.earthquake.value]["df"],
                    group_by_key="ROW",
                )
                result_df = calculate_evacuation_impacted(
                    source_df=result_df,
                    evacuation_df=data_layers[Events_Type.evacuation.value]["df"],
                    group_by_key="ROW",
                )
        ...
        def Calculate_AllEventsPopJoin(events_df, population_df):
            ...
            try:
                ...
                sum_sdf = result_df.groupby("objectid_join")[
                    "L0_over55",
                    "L0_over65",
                    "L0_over75",
                    "L0_over85",
                    ...
                    "L0_cdss_ihss_recpts",
                    "L0_cdph_wic_partipts",
                    "L0_cdph_medicare_benes",
                    "L0_cdph_medicare_dme",
                    "L0_dhcs_medi_cal_benes",
                    "L0_dhcs_medi_cal_dme",
                    "L0_cdds_clients",
                ].sum()
        ...
        if no_errors(Log_Errors):
            try:
                ...
                if all_events_df.empty is True:
                    ...
                    result_df = local_join_df(
                        left_df=facilities_df,
        ...
        if no_errors(Log_Errors):
            try:
                ...
                if all_events_df.empty is True:
                    ...
                    event_types = [
                        "PSPS_Active_Impacted",
                        "PSPS_Potential_Impacted",
                        "PSPS_Reenergized" "Fire",
                        "Earthquake",
                        "Evacuation_Order",
                        "Evacuation_Warning",
                        "Clear_to_Repopulate",
        ...
        if no_errors(Log_Errors):
            try:
                ...
                if all_events_df.empty:
                    ...
                    result_df["SHAPE"] = result_df["SHAPE"].apply(
                        lambda geometry: Point(
                            {
        ...
        if not (result_df is None):
            try:
                ...
                outcome = OverwriteFS.getFeatureServiceTarget(
                    view_item, verbose=None, outcome=None, ignoreDataItemCheck=False
                )
                ...
                outcome = OverwriteFS.swapFeatureViewLayers(
                    view_item,
                    updateFile=target_filepath,
                    touchItems=True,
                    verbose=True,
                    touchTimeSeries=False,
                    outcome=None,
                    noIndexes=False,
                    preserveProps=True,
                    noWait=False,
        ...
        if no_errors(Log_Errors):
            try:
                ...
                pop_score_df = pop_df.melt(
                    [
                        "OBJECTID",
                        pop_zipcode_fieldname,
                        pop_region_fieldname,
                        pop_total_fieldname,
                        pop_shape_fieldname,
                    ],
                ...
                pop_score_df["pop_total"] = pop_score_df.apply(
                    lambda row: result.loc[row[pop_type_fieldname]][pop_sub_fieldname],
                    axis=1,
                )
            ...
            try:
                ...
                print(
                    "Appending {} dummy records to {} with {} records.".format(
                        len(pop_county_df.index), layer_population, len(pop_df.index)
                    )
                )

                temp_series = []
                mod_fields = [
                    {"name": "PSPS_Active_Impacted", "value": "YES"},
