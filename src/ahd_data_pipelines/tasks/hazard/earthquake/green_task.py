...
class DashboardEarthquakeEpicenterGreenTask(MedallionETLTask):
    def __init__(self, spark: SparkSession = None, init_conf: dict = None):
        super(DashboardEarthquakeEpicenterGreenTask, self).__init__(
            spark=spark, init_conf=init_conf
        )

    def custom_transform(self, dataFrames: array, params: dict = None) -> array:
        print(
            f"Doing custom transformation of earthquake data with {dataFrames[0].count()} records"
        )
        ...
        print(
            f"Finished custom transformation of DashboardEarthquakeEpicenterGreenTask with {len(gpd_df)} records"
        )
