...
class PandasHelper():
    ...
    @staticmethod
    def pysparksql_to_pandas_or_geopandas(pysparksql_df: pyspark.sql.DataFrame = None) -> gpd.GeoDataFrame:
    ...
    @staticmethod
    def pysparksql_to_pandasDf(pysparksql_df: pyspark.sql.DataFrame = None) -> pd.DataFrame:
        if "true" == os.environ.get('CI'):
            if not isinstance(pysparksql_df, pyspark.sql.DataFrame):
                raise ValueError(f'Unexpected type;{type(pysparksql_df)}')
        elif "true" != os.environ.get('LOCAL'):
